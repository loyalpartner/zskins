...
pub fn input_key_bindings() -> Vec<KeyBinding> {
    vec![
        ...
        KeyBinding::new("ctrl-w", DeleteWord, Some("TextInput")),
        KeyBinding::new("ctrl-u", ClearLine, Some("TextInput")),
        KeyBinding::new("ctrl-k", ClearLine, Some("TextInput")),
        // Word movement
...
impl TextInput {
    ...
    fn previous_word_boundary(&self, offset: usize) -> usize {
        self.content[..offset]
            ...
            .rev()
            .find(|&(_, w)| w.chars().next().is_some_and(|c| c.is_alphanumeric()))
            .map(|(i, _)| i)
            .unwrap_or(0)
    }
    ...
    fn next_word_boundary(&self, offset: usize) -> usize {
        ...
        for (i, w) in self.content[offset..].split_word_bound_indices() {
            let is_word = w.chars().next().is_some_and(|c| c.is_alphanumeric());
...
// Custom element for rendering the text with cursor and selection.
...
impl Element for TextElement {
    ...
    fn paint(
        ...
        let line = prepaint.line.take().expect("line must be set by prepaint");
        let _ = line.paint(
            bounds.origin,
            window.line_height(),
            gpui::TextAlign::Left,
